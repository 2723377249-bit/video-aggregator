// Cloudflare Pages Function：同源代理 Backblaze B2 私有桶 + 公开聚合 + 视频上传
// 与 worker.js 逻辑一致，由 Cloudflare Pages 托管。
...
async function authorize(env) {
  ...
  const d = await fetch(ad.apiUrl + "/b2api/v2/b2_get_download_authorization", {
    ...
    body: JSON.stringify({ bucketId: env.B2_BUCKET_ID, fileNamePrefix: "", validDurationInSeconds: 604800 }),
  });
...
async function handleUpload(request, env, origin) {
  ...
  if (!/mp4|octet-stream/i.test(ctype)) return jsonResponse({ message: "仅支持 mp4 视频" }, 415, origin);
  let buf;
  try {
  ...
  if (buf.byteLength === 0) return jsonResponse({ message: "文件为空" }, 400, origin);
  if (buf.byteLength > MAX_UPLOAD) return jsonResponse({ message: "文件超过 80MB 上限" }, 413, origin);
  ...
  const entry = { id: base, file: base + ".mp4", poster: "", label, desc: "用户上传", source: "upload", seed: false };
  try {
...
async function handleAggregate(request, env, origin) {
  ...
  try {
    if (env.AGGREGATE_KV) {
      ...
      if (recent.length >= 3) return jsonResponse({ message: "提交太频繁，请 1 分钟后再试" }, 429, origin);
      recent.push(now);
  ...
  const rawUrls = Array.isArray(body.urls) ? body.urls : typeof body.url === "string" ? [body.url] : [];
  if (rawUrls.length === 0) return jsonResponse({ message: "缺少 url / urls 参数" }, 400, origin);
  if (rawUrls.length > 5) return jsonResponse({ message: "一次最多提交 5 个链接" }, 400, origin);
  const urls = validateUrls(extractUrls(rawUrls.join("\n")));
  if (urls.length === 0) return jsonResponse({ message: "未识别到 B站/抖音 链接" }, 400, origin);
  if (!env.GITHUB_TOKEN) return jsonResponse({ message: "服务端未配置 GITHUB_TOKEN，无法触发自动汇总" }, 503, origin);
  ...
  const dispatch = await fetch(`${GITHUB_API}/repos/${repo}/dispatches`, {
    ...
    body: JSON.stringify({ event_type: "aggregate", client_payload: { urls, submitted_at: new Date().toISOString() } }),
  });
  if (!dispatch.ok) {
    const txt = await dispatch.text();
    return jsonResponse({ message: "触发 GitHub Actions 失败：" + dispatch.status + " " + txt.slice(0, 200) }, 502, origin);
  }
...
export async function onRequest(context) {
  ...
  if (path === "/api/upload") return handleUpload(request, env, origin);
  if (path === "/api/aggregate") return handleAggregate(request, env, origin);
  ...
  const upstream = auth.dl + "/file/" + env.B2_BUCKET + target + "?Authorization=" + auth.token;
  const headers = {};
